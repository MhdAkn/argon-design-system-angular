...
export class SignupComponent implements OnInit {
  ...
  onFormSubmit() {
    ...
    if (this.signUpForm.invalid) {
      ...
      this._alertService.showToast('Formulaire invalide! Veuillez remplir tout les champs', 'error', 'top-center')
      return;
    }
    ...
    this.authService.registration(this.user).subscribe({
      next: (res) => {
        ...
        if (res.status == HttpCode.SUCCESS) {
          ...
          this._alertService.showToast('Inscription échouée', 'error', 'top-center')
        }
      }
    })
